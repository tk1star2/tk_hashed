...
#iters = [500, 1000, 10500, 11000, 500, 1000, 1000, 1000, 1000, 1000] #max iteration of each stage
...
layer {
  ...
  data_param {
    source: "examples/hash/mnist_train_lmdb"
    batch_size: 50
    backend: LMDB
  }
}
layer {
  ...
  data_param {
    source: "examples/hash/mnist_test_lmdb"
    batch_size: 50 
    backend: LMDB
  }
}
...
# param first : filter
# param second : biases
# wegiht filter : type is for initialized method - xavier
# bias filter : type is for intitialized method - constant
...
def generate_drop_layer(dropout_ratio, layer_name, bottom, top):
    drop_layer_str = '''
layer {
  name: "%s"
  type: "Dropout"
...
}'''%(layer_name, bottom, top, dropout_ratio)
...
def generate_lenet(stage):
    ...
    if stage >= 3:
      ...
      ratio = 0
    network_str += generate_cmp_fc_layer(1000,"fc1","data","fc1","xavier",ratio,9800,quantize_term)
    network_str += generate_activation_layer("relu1", "fc1", "fc1", "ReLU")
    network_str += generate_drop_layer(0.25, "drop1", "fc1", "fc1")
    if stage >= 4:
      ratio = sparse_ratio_vec[3]
    else:
      ratio = 0
    network_str += generate_cmp_fc_layer(10,"fc2","fc1","fc2","xavier",ratio,125,quantize_term)
    network_str += generate_drop_layer(0.25, "drop2", "fc2", "fc2")
    network_str += generate_softmax_loss("fc2")

    protoname = 'examples/hash/lenet_train_test_compress_stage%d.prototxt' %stage
    fp = open(protoname, 'w')
...
net: "examples/hash/lenet_train_test_compress_stage%d.prototxt"
...
snapshot: 500 
snapshot_prefix: "examples/hash/models/lenet_finetune_stage%d" 
# solver mode: CPU or GPU
solver_mode: GPU 

''' %(stage, max_iter, stage ) 
    protoname = 'examples/hash/lenet_solver_stage%d.prototxt' %stage
    fp = open(protoname,'w')
    fp.write(solver_str)
    fp.close()
...
if __name__ == '__main__':
    ...
    for s in range(4,max_stage):
      ...
      cmd = "./build/tools/caffe train --solver=examples/hash/lenet_solver_stage%d.prototxt --weights=examples/hash/models/%s " %(s+1, modelfile)
      #print cmd
      os.system(cmd)      
